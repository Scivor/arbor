...
class CheapTimesFM:
    """
    Lightweight local fallback when `timesfm` is unavailable.

    It extrapolates a simple capped linear trend from recent prices and
    synthesizes quantile bands around that baseline so downstream code can
    still exercise the forecast path.
    """

    def forecast(self, inputs: list[np.ndarray], horizon: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        if series.size >= 6:
            recent_returns = np.diff(series[-6:]) / np.maximum(series[-6:-1], 1e-10)
            step_change = float(np.clip(np.mean(recent_returns), -0.02, 0.02))
            volatility = float(np.clip(np.std(recent_returns), 0.01, 0.08))
        else:
            step_change = 0.0
            volatility = 0.02

        steps = np.arange(1, h + 1, dtype=np.float64)
        median = last_price * (1.0 + step_change * steps)
        bands = np.array([2.0, 1.35, 0.75, 0.35, 0.0, 0.35, 0.75, 1.35, 2.0], dtype=np.float64)

        quantiles = []
        for band in bands:
            direction = -1.0 if len(quantiles) < 4 else 1.0
            if band == 0.0:
                quantiles.append(median.copy())
                continue
            width = last_price * volatility * np.sqrt(steps) * band
            quantiles.append(np.maximum(median + direction * width, 1e-6))

        point = median.reshape(1, h)
        full = np.vstack(quantiles)
        return point, full
