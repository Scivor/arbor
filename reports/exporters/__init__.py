...
def _rich_export(report: "Report", dest: Optional[str] = None) -> None:
    # Rich goes to console only
    from reports.exporters.rich_exporter import export_rich
    export_rich(report)


def _pdf_export(report: "Report", dest: Optional[str] = None) -> None:
    dest = dest or "coffee_outlook.pdf"
    from reports.exporters.pdf_exporter import export_pdf
    export_pdf(report, dest)
...
def _register_builtin():
    ...
    _FORMATTERS = {
        ...
        "rich": _InlineExporter("rich", "txt", "text/plain", _rich_export),
...
def get_exporter(name: str) -> ExporterPlugin:
    """Get an exporter by name (e.g. 'text', 'json', 'pdf')."""
    if not _FORMATTERS:
        _register_builtin()
...
def export_report(
    ...
    Args:
        ...
        format:  Exporter name ('text', 'json', 'rich', 'pdf').
        dest:    Output path (optional).

    Returns:
        String output for text/json, None for rich/pdf.
    """
